...
class atractionModel extends Model
{
    ...
    public function getAtractions()
    {
        $query = $this->db->table($this->table)
            ...
            ->join('category_atraction', 'category_atraction.id = atraction.category_atraction_id')
            ->get()->getResult();
        return $query;
    }
    ...
    public function getAtractionStone()
    {

        $query = $this->db->table($this->table)
            ->select("{$this->columns},{$this->coords},{$this->geom_area}")
            ->join('category_atraction', 'category_atraction.id = atraction.category_atraction_id')
            ->where("atraction.id='02' OR atraction.id='03' OR atraction.id='04' ORDER BY atraction.id ")
            ->get()->getResult();
        return $query;
    }
    ...
    public function getAtractionByCategory($category)
    {
        $query = $this->db->table($this->table)
            ->select("{$this->columns},{$this->coords},{$this->geom_area}")
            ->join('category_atraction', 'category_atraction.id = atraction.category_atraction_id')
            ->where('atraction.category_atraction_id=', $category)
            ->get();
        return $query;
    }
